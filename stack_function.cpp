...
enum number_error StackPop (struct stack_t *stk, stack_elem_t *value){
    ...
    *value = stk->data[stk->size]; 
    ...
    if (stk->size < (stk->capacity) / 4){   //3 && stk->capacity % 3
        stk->capacity = (stk->capacity) / 4;   
        stk->data = (stack_elem_t*) realloc (stk->data - 1, stk->capacity * sizeof(stack_elem_t) + 2 * sizeof(canary2));
